...
VelodyneStreamer::VelodyneStreamer(std::string pcap) {
	...
	while (_reader.nextPacket(packet)) {
		if (packet.size() >= 1248) {
			std::cout << (int)packet.data().payload()[1247 - 42] << std::endl;
...
bool VelodyneStreamer::nextFrameHDL64(VCloud& cloud) {
	...
	std::vector<int> laser_ids64 = HDL_laser_arrangements();
		...
		for (int n = 0; n < 12; n++) {
			...
			for (int i = 0; i < 32; i++) {
				...
				if (distance > 0) {
					...
					p.laser_id = laser_ids64[i + !upper * 32];
					...
					//	minIntensity = arr[j][MININTENSITY];
					...
					//	float focalslope = arr[j][FOCALSLOPE];
...
bool VelodyneStreamer::nextFrameHDL64(pcl::PointCloud<pcl::PointXYZRGBNormal>& cloud, std::vector<TData>& data) {
  ...
  while (_reader.nextPacket(packet)) {
    ...
    //std::ofstream ofs;
    //ofs.open("C:\\Users\\bence\\Desktop\\DATA64.txt", std::ios::app);
    ...
    for (int n = 0; n < 12; n++) {
      ...
      for (int i = 0; i < 32; i++) {
        ...
        if (distance > 0) {
          ...
          //ofs << atan2f(p.z, sqrtf(p.x * p.x + p.y * p.y)) * 180.0f / M_PI << " ";
          cloud.push_back(p);
        }
        else {
          //ofs << "null ";
...
std::vector<int> VelodyneStreamer::HDL_laser_arrangements() {
	std::vector<double> elevations;
	std::vector<double> sorted_elevations;
	std::vector<int> id_list(64);
	for (auto i = 0; i < 64; i++) {
		sorted_elevations.push_back(arr[i][1]);
		elevations.push_back(arr[i][1]);
	}

	std::sort(sorted_elevations.begin(), sorted_elevations.end());

	for (auto i = 0; i < 64; i++) {
		for (auto j = 0; j < 64; j++) {
			if (sorted_elevations[j] == elevations[i]) {
				id_list[i] = j;
				break;
			}
		}
